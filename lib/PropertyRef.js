...
class PropertyRef {
  ...
  _parse(str, builder) {
    ...
    if (parts.length === 1) {
      ...
      this.propertyName = eagerColumn;
      this.modelClass = this.relation.relatedModelClass;
    } else {
      utils.throwError('PropertyRef: only two levels of relations is supported');
  ...
  buildFilter(param, builder, boolOp) {
    ...
    if (rel && !rel.isOneToOne()) {      
      const subQuery = rel.ownerModelClass.relatedQuery(rel.name).alias(rel.relatedModelClass.tableName);
